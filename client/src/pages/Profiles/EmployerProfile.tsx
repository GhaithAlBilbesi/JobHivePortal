...
const EmployerProfile: React.FC = () => {
  ...
  const uploadImage = async (file: File, type: 'logo' | 'banner') => {
    ...
    const res = await fetch('http://localhost:5000/api/employer/upload-image', {
      method: 'POST',
      body: formData,
    });
...
const handleChangePassword = async () => {
  ...
  try {
    ...
    try {
      await fetch('http://localhost:5000/api/employer/company-info', {
        method: 'POST',
      ...
      await fetch('http://localhost:5000/api/employer/founding-info', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_id: userId,
      ...
      await fetch('http://localhost:5000/api/employer/contact-info', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_id: userId,
      ...
      if (formData.social_links) {
        ...
        await fetch('http://localhost:5000/api/employer/social-media', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  <Card>
    {isEditing ? (
      ...
      companyInfo.banner_url && (
        <img
          src={`http://localhost:5000${companyInfo.banner_url}`}
          alt="Company Banner"
    ...
    <CardHeader className="flex flex-col sm:flex-row items-center gap-6">
      <Avatar className="w-32 h-32 border-4 border-[#F6C500]">
        <AvatarImage src={`http://localhost:5000${companyInfo.logo_url}`} />
